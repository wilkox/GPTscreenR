...
retrieve_abstract_DOI <- function(DOI, cache_file = NULL) {
  ...
  result <- data.frame(DOI = DOI, abstract = NA_character_, abstract_source = NA_character_)
  if (! is.null(cache_file)) {
    if (fs::file_exists(cache_file)) {
      abstracts_cache <- readRDS(cache_file)
      abstracts_cache <- dplyr::bind_rows(abstracts_cache, result)
...
```{r}
qources <- sources %>% filter(! is.na(abstract))
sources <- screen_sources(sources, study_description, n = 324,
