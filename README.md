...
By default, GPTscreenR uses the OpenAI chat API with the `gpt-4o` model.
Alternative LLM services, including the OpenAI assistants API and local
LLMs with ollama, can be selected using the `service` argument to
`screen()`. The model can be set with the `model` argument to
`screen()`.
